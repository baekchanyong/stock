...
if 'kospi_df' not in st.session_state: st.session_state.kospi_df = pd.DataFrame()
...
if st.session_state.kospi_df.empty:
    with st.spinner("KOSPI 로딩 중..."): st.session_state.kospi_df = load_kospi_data()
...
def analyze_stock(ticker, name, current_price, shares, marcap_rank):
    ...
    if is_pref:
        return None, {"시총순위": marcap_rank, "종목명": name, "현재주가": current_price, "제외사유": "우선주 제외"}
    ...
    except: return None, {"시총순위": marcap_rank, "종목명": name, "현재주가": current_price, "제외사유": "재무 데이터 로드 오류"}
    ...
    except: return None, {"시총순위": marcap_rank, "종목명": name, "현재주가": current_price, "제외사유": "재무 데이터 파싱 오류"}

    if a_eps == 0 or bps == 0 or t_equity == 0: return None, {"시총순위": marcap_rank, "종목명": name, "현재주가": current_price, "제외사유": "필요 데이터 누락"}
    ...
    data = {"시총순위": marcap_rank, "종목명": name, "현재주가": current_price, "적정주가(10)": p10, "목표주가(10)": t10, "괴리율(10)": ((p10 - current_price) / current_price) * 100 if p10 > 0 else 0, "적정주가(15)": p15, "목표주가(15)": t15, "EPS": a_eps, "BPS": bps, "부채비율(%)": d_ratio, "총부채_원": t_debt, "유동부채_원": c_liab, "총자본_원": t_equity, "상장주식수_원": shares}
    if reason:
        data["제외사유"] = reason
        return (None, data)
    return (data, None)
...
if not kospi_df.empty:
    ...
    with col1:
        search_mode = st.radio("모드를 선택하세요:", ("KOSPI 전체 탐색", "KOSPI 상위 N개 탐색", "사용자 지정 탐색"))
    with col2:
        if search_mode == "KOSPI 상위 N개 탐색":
            top_n = st.number_input("탐색 개수:", min_value=1, max_value=len(kospi_df), value=50)
    ...
    if st.session_state.running:
        ...
        for i, stock in enumerate(st.session_state.target_stocks[st.session_state.current_idx:], start=st.session_state.current_idx):
            ...
            res, extra = analyze_stock(stock['Code'], stock['Name'], float(stock['Close']), float(stock['Stocks']), stock['Marcap_Rank'])
